...
export function PromptCard({
  ...
  const typeIcons = {
    prompt: <MessageSquare size={18} className="text-purple-500" />,
    command: <Terminal size={18} className="text-green-500" />,
  ...
  if (view === 'gallery') {
    return <div onClick={() => onOpenPrompt(prompt)} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all cursor-pointer group">
        <div className="p-5">
          ...
          <h3 className="font-medium mb-2 text-slate-800 group-hover:text-indigo-600 transition-colors">
            {prompt.title}
          </h3>
        ...
        <div className="bg-slate-50 p-4 flex justify-between items-center border-t border-slate-200">
          ...
          <button onClick={handleCopy} className="text-slate-500 hover:text-indigo-600 p-1.5 bg-white rounded-md border border-slate-200 shadow-sm hover:shadow transition-all">
            <Copy size={14} />
          </button>
        </div>
      </div>;
